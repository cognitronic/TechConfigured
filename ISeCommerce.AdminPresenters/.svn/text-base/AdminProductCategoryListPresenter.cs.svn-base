...
{
    public class AdminProductCategoryListPresenter : Presenter
    {
        IAdminProductCategoryListView _view;

        public AdminProductCategoryListPresenter(IAdminProductCategoryListView view, ISessionProvider session)
            : base(view, session)
        {
            _view = base.GetView<IAdminProductCategoryListView>();
        ...
        void _view_OnItemSelected(object sender, IdeaSeedLinkButtonArgs e)
        {
            if (!string.IsNullOrEmpty(e.Name))
                _view.NavigateTo(SecurityContextManager.Current.CurrentURL + "/" + e.Name + "/Details");
        }
        ...
        {
            _view.ResultSet = _view.CachedProductCategories;
